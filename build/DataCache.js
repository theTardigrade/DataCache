(function () {
	...
	(function (global, module, Error, TypeError, D, M, O, N) {
		...
		var IS_NODE = typeof module === OBJECT_TYPE && typeof module.exports === OBJECT_TYPE && typeof process === OBJECT_TYPE && typeof process.versions == OBJECT_TYPE && !isNaN(parseFloat(process.versions.node, 10));
		...
		var exists = (function (data) {
			...
			return o;
		})([{ key: "assign" }, { key: "defineProperty" }, { key: "freeze" }, { key: "now", object: D }]);
		...
		var search = function (cacheArray, key) {
			...
			for (;;) {
				...
				if (key < cacheArray[midpoint]) upperBound = midpoint - 2;else if (key > cacheArray[midpoint]) lowerBound = midpoint + 2;
			}
		};
		...
		function DataCache(options) {
			...
			this.get = function (key, options) {
				...
				if (typeof key !== getDefinedProperty("keyType")) return value;
				...
				if (options) {
					...
					if (options.metadataOnly) {
						...
						for (var vKey in value) {
							if (vKey !== "data") metadata[vKey] = value[vKey];
						}return metadata;
					}

					if (options.dataOnly) {
						if (optionCount) throw new Error("The \"dataOnly\" and \"metadataOnly\" options are mutually contradictory.");
			...
			this.set = function (key, data) {
				...
				if (typeof keyType !== STRING_TYPE) setDefinedProperty("keyType", keyType = typeof key);

				if (typeof key !== keyType) throw new TypeError("Key must be a " + keyType + ".");

				var index = search(cache, key);

				if (index === -1) index = cache.length + 1;

				if (index >= getDefinedProperty("capacity") * 2) index = getDefinedProperty("_oldestIndex");

				if (typeof data === OBJECT_TYPE && exists.freeze) O.freeze(data);
				...
				object.created = cache[index] && cache[index].created ? cache[index].created : object.updated;
			...
			this.unset = function (key) {
				if (typeof key !== getDefinedProperty("keyType")) return false;
			...
			this.iterate = function (callback, options) {
				...
				for (var i = 0, l = cache.length; i < l; i += 2) {
					...
					if (typeof key !== keyType) continue;
			...
			var getFallbackDefinedPropertyName = function (prefix, prop) {
				...
				if (exists.defineProperty) {
					...
					for (var i = 0, l = keys.length, k; i < l; ++i) {
						k = keys[i] + "et";
						if (typeof options[k] === FUNCTION_TYPE) _this[getFallbackDefinedPropertyName(k, prop)] = options[k];
					}
				}
			},
			    getDefinedProperty = function (prop) {
				return exists.defineProperty ? _this[prop] : _this[getFallbackDefinedPropertyName("get", prop)]();
			},
			    setDefinedProperty = function (prop, value) {
				return exists.defineProperty ? _this[prop] = value : _this[getFallbackDefinedPropertyName("set", prop)](value);
			};
			...
			definePropertyHere("keyType", {
				...
				set: (function () {
					...
					for (var i = 0, l = ALLOWABLE_KEY_TYPES.length, t; i < l; ++i) {
						...
						errorMessage += i < l - 2 ? t + ", " : i < l - 1 ? t : " and " + t + ".";
					}

					return function (keyType) {
						var isTypeAllowable = (function () {
							for (var _i2 = 0, _l = ALLOWABLE_KEY_TYPES.length; _i2 < _l; ++_i2) {
								if (keyType === ALLOWABLE_KEY_TYPES[_i2]) return true;
							}return false;
						})();

						if (keyType === privateKeyType) return;

						if (isTypeAllowable) privateKeyType = keyType.toLowerCase();else throw new TypeError(errorMessage);
					};
				})()
			});

			if (options && typeof options.keyType !== UNDEFINED_TYPE) setDefinedProperty("keyType", options.keyType);
			...
			definePropertyHere("capacity", {
				...
				set: function (capacity) {
					...
					var value = capacity !== NUMBER_TYPE && !isNaN(capacity) ? parseInt(capacity, 10) : capacity;

					value = M.min(value, MAX_ARRAY_LENGTH);

					if (isNaN(value)) throw new TypeError("Suggested capacity cannot be parsed.");
			...
			setDefinedProperty("capacity", options && typeof options.capacity !== UNDEFINED_TYPE ? options.capacity : MAX_ARRAY_LENGTH);
		...
		(function (prototype) {
			...
			if (exists.assign) O.assign(DataCache.prototype, prototype);else for (var key in prototype) {
			...
			map: function (callback, options) {
				...
				this.iterate((function (key, value) {
					...
					if (!options || !options.dataOnly) newValue = newValue.data;

					if (typeof newValue !== UNDEFINED_TYPE) _this2.set(key, newValue);
				}), options);
			},
			...
			filter: function (callback, options) {
				...
				this.iterate((function (key, value) {
					if (!callback(key, value)) filteredKeys.push(key);
				}), options);
			...
			isFull: function () {
				return exists.defineProperty ? this.size === this.capacity : this.getSize() === this.getCapacity();
			},
	...
	})(typeof window === OBJECT_TYPE ? window : typeof global === OBJECT_TYPE ? global : this, this.module, Error, TypeError, Date, Math, Object, Number);
})();